...
router.post('/login', async(req, res) => {
    try{
        ...
        const normalizedAadharCardNumber = String(aadharCardNumber ?? req.body.aadhar ?? req.body.aadhaarNumber ?? req.body['Aadhar Card Number'] ?? '').replace(/\D/g, '').trim();

        // Check if aadharCardNumber or password is missing
        if (!normalizedAadharCardNumber || !password) {
            return res.status(400).json({ error: 'Aadhar Card Number and password are required' });
        }

        // Find the user by aadharCardNumber
        const user = await User.findOne({ AadharCardNumber: normalizedAadharCardNumber });
